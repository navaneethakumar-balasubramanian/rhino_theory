...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div([
            ...
            html.Div([
                html.H5('window', id='window-title', className='column'),
                dcc.Slider(
                    id='window-slider',
                    min=10,
                    max=200,
...
def update_figure(alpha, rho, window):
    ...
    rock = Rock(alpha, rho)
    wavelet = TheoreticalWavelet(
        pipe, rock, frequencies=f)

    frequency_domain_primary = wavelet.amp_phase2complex(*wavelet.primary_in_frequency_domain)
    frequency_domain_reflected = wavelet.amp_phase2complex(*wavelet.reflected_in_frequency_domain)
    ...
    reflected_real = go.Scatter(y=frequency_domain_reflected.real, marker=dict(color='green'))
    reflected_imag = go.Scatter(y=frequency_domain_reflected.imag, marker=dict(color='green'))

    primary_nyquist = go.Scatter(x=frequency_domain_primary.real, y=frequency_domain_primary.imag, marker=dict(color='red'))
    reflected_nyquist = go.Scatter(x=frequency_domain_reflected.real, y=frequency_domain_reflected.imag, marker=dict(color='green'))

    primary_wavelet_full = go.Scatter(y=wavelet.primary_in_time_domain(None), marker=dict(color='red'))
    reflected_wavelet_full = go.Scatter(y=wavelet.reflected_in_time_domain(None), marker=dict(color='green'))
    multiple_wavelet_full = go.Scatter(y=wavelet.multiple_in_time_domain(None), marker=dict(color='blue'))

    time_sampling_window = np.arange(
        0-(window/2*wavelet.time_sampling),
        0+(window/2*wavelet.time_sampling)+wavelet.time_sampling, wavelet.time_sampling) * 1000

    primary_wavelet = go.Scatter(x=time_sampling_window, y=wavelet.primary_in_time_domain(window), marker=dict(color='red'))
    reflected_wavelet = go.Scatter(x=time_sampling_window,y=wavelet.reflected_in_time_domain(window), marker=dict(color='green'))
    multiple_wavelet = go.Scatter(x=time_sampling_window,y=wavelet.multiple_in_time_domain(window), marker=dict(color='blue'))
